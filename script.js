...
/* ═══════════════════════════════════════════════
   ...
   précis sur chaque événement qui n'en a pas encore.
   ═══════════════════════════════════════════════ */
const __LP = {
  ...
  202:"Poitiers, Vienne, France",203:"Azincourt, Pas-de-Calais, France",
  204:"Troyes, Aube, France",205:"Assemblée nationale, Paris, France",
  ...
  254:"Palerme, Sicile, Italie",255:"Col du Mont-Cenis, Alpes, Italie",
  256:"Cannae, Apulie, Italie",257:"Carthage, Tunisie",
  258:"Athènes, Grèce",259:"Athènes, Grèce",260:"Chios, Grèce",
  261:"Qadesh (Tell Nebi Mend), Homs, Syrie",
  ...
  335:"Wassy, Amboise et Paris, France",336:"Gravelines, Nord, France",
  337:"Padoue, Vénétie, Italie",
  ...
  352:"Musée du Louvre, Paris, France",353:"Rainhill, Merseyside, Angleterre",
  ...
  365:"Champ de Mars, Paris, France",366:"San Francisco, Californie, États-Unis",
  ...
  385:"Washington D.C., États-Unis",386:"Place Tian'anmen, Pékin, Chine",
...
function goToEv(id){
  ...
  document.getElementById('sres').classList.remove('open');
  if(currentMapMode){
    // En mode carte : centrer la carte sur l'événement
    setMode('frise');
  }
...
function shareApp(){
  ...
  if(navigator.share){ navigator.share({title:'Ma Frise Chronologique',text:'500 événements historiques !',url}); }
...
wrap.addEventListener('wheel',e=>{ e.preventDefault(); const f=e.deltaY<0?1.18:1/1.18,mx=e.clientX-wrap.getBoundingClientRect().left,yAtMouse=xToYear(mx); scale*=f; const ds=defScale(); if(scale<ds*0.98) scale=ds; offsetX=mx-80-(yAtMouse-getRange().minY)*scale; render(); updZoom(); },{passive:false});
let ltx=null,ltd=null,ltcx=null;
wrap.addEventListener('touchstart',e=>{ if(e.touches.length===2){ ltd=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY); ltcx=(e.touches[0].clientX+e.touches[1].clientX)/2-wrap.getBoundingClientRect().left; ltx=null; } else{ ltx=e.touches[0].clientX; ltd=null; } },{passive:true});
wrap.addEventListener('touchmove',e=>{ if(e.touches.length===2&&ltd!==null){ const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY); const cx=(e.touches[0].clientX+e.touches[1].clientX)/2-wrap.getBoundingClientRect().left; const yAC=xToYear(ltcx); scale*=d/ltd; const ds=defScale(); if(scale<ds*0.98) scale=ds; ltd=d; ltcx=cx; offsetX=cx-80-(yAC-getRange().minY)*scale; render(); updZoom(); } else if(ltx!==null&&e.touches.length===1){ offsetX+=e.touches[0].clientX-ltx; ltx=e.touches[0].clientX; render(); } },{passive:true});
wrap.addEventListener('touchend',()=>{ ltx=null; ltd=null; ltcx=null; });
document.addEventListener('keydown',e=>{ if(e.key==='Escape'){ closeMod(); closeCard(); closeClusterList(); clearSearch(); } if((e.metaKey||e.ctrlKey)&&e.key==='k'){ e.preventDefault(); document.getElementById('si').focus(); } if((e.metaKey||e.ctrlKey)&&e.key==='Enter'&&document.getElementById('mbg').classList.contains('open')) saveEv(); });
document.addEventListener('click',e=>{ if(!document.getElementById('sres').contains(e.target)&&e.target!==document.getElementById('si')) document.getElementById('sres').classList.remove('open'); if(!document.getElementById('region-results').contains(e.target)&&e.target!==document.getElementById('region-input')) document.getElementById('region-results').style.display='none'; });
window.addEventListener('resize',()=>{ if(currentEra) resetView(); });

/* ═══════════════════════════════════════════════════════
   CARTE MONDE — Leaflet
   Geocodage des lieux via dictionnaire statique de coordonnées
   ═══════════════════════════════════════════════════════ */

let mapInstance=null;
let mapMarkers=L?L.layerGroup():null;
let mapCatFilter='all';

/* Dictionnaire statique de coordonnées pour les lieux connus */
const GEO_DICT = {
  // France
  ...
  "Verdun":                 [49.1604, 5.3877],
  "Normandie":              [49.1829, -0.3707],
  "Bordeaux":               [44.8378, -0.5792],
  "Toulouse":               [43.6047, 1.4442],
  "Strasbourg":             [48.5734, 7.7521],
  "Dunkerque":              [51.0343, 2.3767],
  "Compiègne":              [49.4183, 2.8228],
  "Reims":                  [49.2583, 4.0317],
  "Azincourt":              [50.4600, 2.1400],
  "Poitiers":               [46.5802, 0.3404],
  "Waterloo":               [50.7150, 4.3990],
  "Amboise":                [47.4107, 0.9821],
  "Fontainebleau":          [48.4045, 2.7024],
  "Troyes":                 [48.2973, 4.0744],
  "Varennes":               [49.2237, 5.0330],
  "Avignon":                [43.9493, 4.8055],
  "Crecy":                  [50.2569, 1.8819],
  // Europe
  ...
  "Byzance":                [41.0082, 28.9784],
  "Moscou":                 [55.7558, 37.6173],
  "Saint-Pétersbourg":      [59.9343, 30.3351],
  "Kyiv":                   [50.4501, 30.5234],
  "Srebrenica":             [44.0050, 19.2983],
  ...
  "Waterloo, Belgique":     [50.7150, 4.3990],
  "Austerlitz":             [49.1700, 16.7600],
  "Hastings":               [50.8548, 0.5730],
  "Oxford":                 [51.7520, -1.2577],
  "Cambridge":              [52.2053, 0.1218],
  "Glasgow":                [55.8642, -4.2518],
  "Édimbourg":              [55.9533, -3.1883],
  "Bologne":                [44.4949, 11.3426],
  "Florence":               [43.7696, 11.2558],
  "Venise":                 [45.4408, 12.3155],
  "Trente":                 [46.0748, 11.1217],
  "Milan":                  [45.4642, 9.1900],
  "Naples":                 [40.8518, 14.2681],
  "Palerme":                [38.1157, 13.3615],
  "Ravenne":                [44.4184, 12.2035],
  "Gênes":                  [44.4056, 8.9463],
  "Barcelone":              [41.3851, 2.1734],
  "Séville":                [37.3891, -5.9845],
  "Grenade":                [37.1773, -3.5986],
  "Tolède":                 [39.8628, -4.0273],
  "Porto":                  [41.1579, -8.6291],
  // Proche-Orient / Moyen-Orient
  "Jérusalem":              [31.7683, 35.2137],
  "Bagdad":                 [33.3128, 44.3615],
  "Babylone":               [32.5355, 44.4275],
  "Damas":                  [33.5138, 36.2765],
  "Beyrouth":               [33.8938, 35.5018],
  "Téhéran":                [35.6892, 51.3890],
  "Riyadh":                 [24.7136, 46.6753],
  "Riyad":                  [24.7136, 46.6753],
  "La Mecque":              [21.3891, 39.8579],
  "Médine":                 [24.4672, 39.6150],
  "Alexandrie":             [31.2001, 29.9187],
  "Gaza":                   [31.5017, 34.4674],
  "Tel Aviv":               [32.0853, 34.7818],
  "Kaboul":                 [34.5553, 69.2075],
  "Doha":                   [25.2854, 51.5310],
  "Koweït":                 [29.3759, 47.9774],
  "Mossoul":                [36.3350, 43.1189],
  // Afrique
  ...
  "Hanoï":                  [21.0285, 105.8542],
  "Hô Chi Minh-Ville":      [10.8231, 106.6297],
  "Saïgon":                 [10.8231, 106.6297],
  "Phnom Penh":             [11.5564, 104.9282],
  "Manille":                [14.5995, 120.9842],
  "Karachi":                [24.8607, 67.0011],
  "Lahore":                 [31.5204, 74.3587],
  "Islamabad":              [33.6844, 73.0479],
  "Mohenjo-Daro":           [27.3238, 68.1377],
  "Xi'an":                  [34.3416, 108.9398],
  "Nankin":                 [32.0603, 118.7969],
  "Wuhan":                  [30.5928, 114.3055],
  "Hangzhou":               [30.2741, 120.1551],
  "Karakoroum":             [47.1967, 102.8375],
  ...
  "Philadelphie":           [39.9526, -75.1652],
  "New Orleans":            [29.9511, -90.0715],
  "Nouvelle-Orléans":       [29.9511, -90.0715],
  "Hiroshima (USA)":        [34.3853, 132.4553],
  "Pearl Harbor":           [21.3643, -157.9550],
  "Montréal":               [45.5017, -73.5673],
  "Ottawa":                 [45.4215, -75.6972],
  "Toronto":                [43.6532, -79.3832],
  "Mexico":                 [19.4326, -99.1332],
  "Tenochtitlan":           [19.4326, -99.1332],
  "La Havane":              [23.1136, -82.3666],
  ...
  "Porto Seguro":           [-16.4400, -39.0650],
  "Montevideo":             [-34.9011, -56.1915],
  "Port-au-Prince":         [18.5944, -72.3074],
  "Cajamarca":              [-7.1638, -78.5001],
  "Cuzco":                  [-13.5319, -71.9675],
  // Australie / Océanie
  "Sydney":                 [-33.8688, 151.2093],
  "Melbourne":              [-37.8136, 144.9631],
  "Christchurch":           [-43.5320, 172.6306],
  "Botany Bay":             [-34.0023, 151.2195],
  // Lune / Espace
  "Lune":                   [0.6875, 23.4333],
  "Orbite terrestre":       [0, 0],
  // Kazakhstan
  "Baïkonour":              [45.9650, 63.3050],
};

function guessCoords(lieu){
  if(!lieu) return null;
...
function refreshMap(){
  ...
  evs.forEach(ev=>{
    ...
    const popupContent=`<div class="map-popup">
      <div class="map-popup-stripe" style="background:${cat.c};margin:-0px -0px 10px;height:4px;border-radius:4px 4px 0 0"></div>
      ...
      <div class="map-popup-acts">
        <a href="${mapsUrl(ev.lieu||ev.title)}" target="_blank" rel="noopener" style="color:${cat.c}">🗺️ Google Maps</a>
...
function mapOpenCard(id){
  // Fermer le popup Leaflet et ouvrir la fiche
...
function renderSubFrise(){
  ...
  const raw=[],rowLast={};
  subEvs.forEach((ev,i)=>{ const px=sfYearToX(yearFrac(ev.y,ev.m,ev.d)); let row=i%2===0?-1:1,found=false; for(let depth=1;depth<=6&&!found;depth++){ for(const r of [-depth,depth]){ if(!rowLast[r]||px-rowLast[r]>80){ row=r; found=true; break; } } } rowLast[row]=px; raw.push({...ev,row,px}); });
  raw.forEach(ev=>{ const px=ev.px; if(px<-60||px>sfW+60) return; const evCat=CATS[ev.cat]||CATS.autre,above=ev.row<0,depth=Math.abs(ev.row); const maxSl=above?SF_AY-46:SF_H-SF_AY-46,sl=Math.max(0,Math.min(28+depth*18,maxSl)); if(sl<5) return; const tipY=above?SF_AY-sl:SF_AY+sl,by=above?tipY-36:tipY; if(by<3||by+36>SF_H-3) return; const isFav=favorites.has(ev.id),isActive=ev.id===sfActiveId; const label=ev.title.length>22?ev.title.slice(0,21)+'…':ev.title; const bw=Math.min(label.length*6.8+28,180),bx=Math.max(4,Math.min(px-bw/2,sfW-bw-4)); if(isActive) h+=`<rect x="${bx-4}" y="${by-4}" width="${bw+8}" height="44" rx="12" fill="${evCat.c}" opacity=".15"/>`; h+=`<circle cx="${px}" cy="${SF_AY}" r="5" fill="${evCat.c}" opacity=".17" data-sfid="${ev.id}"/>`; h+=`<circle cx="${px}" cy="${SF_AY}" r="3.5" fill="${evCat.c}" stroke="var(--paper2)" stroke-width="1.5" style="cursor:pointer" data-sfid="${ev.id}"/>`; if(isFav) h+=`<text x="${px+5}" y="${SF_AY-4}" font-size="8" fill="#f59e0b" style="pointer-events:none">★</text>`; h+=`<line x1="${px}" y1="${SF_AY+(above?-3:3)}" x2="${px}" y2="${tipY}" stroke="${evCat.c}" stroke-width=".9" stroke-dasharray="2 2.5" opacity=".4"/>`; h+=`<rect x="${bx}" y="${by}" width="${bw}" height="36" rx="8" fill="var(--paper)" stroke="${evCat.c}" stroke-width="${isActive?1.75:.7}" style="cursor:pointer" data-sfid="${ev.id}"/>`; h+=`<rect x="${bx}" y="${by}" width="${bw}" height="4" rx="4" fill="${evCat.c}" opacity=".8" style="pointer-events:none"/>`; if(isFav) h+=`<text x="${bx+bw-8}" y="${by+29}" font-size="8" fill="#f59e0b" text-anchor="middle" style="pointer-events:none">★</text>`; h+=`<text x="${bx+8}" y="${by+17}" font-size="10.5" font-weight="500" fill="${evCat.c}" font-family="'Playfair Display',serif" style="cursor:pointer" data-sfid="${ev.id}">${label}</text>`; h+=`<text x="${bx+8}" y="${by+28}" font-size="9" fill="${evCat.c}" opacity=".65" font-family="'DM Sans',sans-serif" data-sfid="${ev.id}" style="cursor:pointer">${fmtDate(ev)}</text>`; });
  ...
  sfSvgEl.innerHTML=h;
  sfSvgEl.querySelectorAll('[data-sfid]').forEach(el=>{ const id=parseInt(el.dataset.sfid); el.addEventListener('click',e=>{ e.stopPropagation(); sfOpenCard(id,e); }); });
  clearSubClusters();
}
...
function saveEvWithParent(){
  ...
  if(!wasEdit&&parentId){
    ...
    setTimeout(()=>{ if(sfParentId===parseInt(parentId)) renderSubFrise(); else openSubFrise(parseInt(parentId)); },400);
  }
}
